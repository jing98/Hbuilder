  ...
  $(function(){
	...
	$points.click(function(){
  	...
  	function updatePoints(next){
  		//计算出目标圆点的下标targetIntex
  		var targetIntex=0;
  		if(typeof next==="boolean")
  		{
	  		if(next){
	  			targetIntex=index+1;  //[0,imgCount+1]
	  			if(targetIntex===imgCount)	//此时看到的是1.jpg-->第1个圆点 
	  			{
	  				targetIntex=0;
	  			}
	  		}
	  		else{
	  			if(targetIntex===-1)		//此时看到的是5.jpg-->第5个圆点 
	  			{
	  				targetIntex=imgCount-1;
	  			}
	  			targetIntex=index-1;
	  		}
  		}
  		...
  		//给目标圆点添加class='on'
  		$points.eq(targetIntex).addClass("on");
  		//$points[targetIntex].className='on';
  		//将index更新为targetIntex
  		index=targetIntex;
  	}
