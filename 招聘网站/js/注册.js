...
function validateForm() { //定义validateForm方法用于客户端校验
	...
	//校验学历
	/*var graduateNode = byId("graduate"); //获得ID值为graduate的节点对象
	var graduate = graduateNode.value;
	if("--请选择--" == graduate) {
		byId("graduate_span").style.color = "red";
		flag = false;
	}*/
...
/*function checkGraduate(node) { //当鼠标离开节点时调用此方法，验证节点内容是否符合注册规范
	var graduate = node.value;
	if("--请选择--" == graduate) {
		byId("graduate_span").style.color = "red";
	} else {
		byId("graduate_span").style.color = "green";
}*/
